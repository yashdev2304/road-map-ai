...
export const AnalysisResults = ({ data, onReset }: AnalysisResultsProps) => {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      doc.text('Pathr', margin, 35);
      
      doc.setFontSize(11);
      doc.setFont('helvetica', 'normal');
      setColor(colors.lightGray, 'text');
      doc.text('Your Personalized Career Roadmap', margin, 48);
      ...
      doc.setFontSize(8);
      doc.text('Generated by Pathr | ' + new Date().toLocaleDateString(), margin, yPos);
      doc.text('pathr.app', pageWidth - margin - 18, yPos);

      // Generate filename from first career path title
      const sanitizedName = data.career_paths[0]?.title?.replace(/[^a-zA-Z0-9]/g, '-') || 'Career';
      doc.save(`${sanitizedName}_Pathr.pdf`);
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-3">
            <span className="bg-gradient-to-r from-violet-400 via-primary to-fuchsia-400 bg-clip-text text-transparent font-black">Pathr</span>
            <div className="h-6 w-px bg-border" />
            <span className="text-xl font-medium text-muted-foreground">Career Blueprint</span>
          </h1>
